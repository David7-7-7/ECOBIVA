...
async function obtenerPorOrden(req, res) {
  try {
    const diagnostico = await diagnosticoDao.obtenerPorOrden(req.params.idOrden);
    if (!diagnostico) {
      return res.status(404).json({ error: "Esta orden todavía no tiene diagnóstico." });
    }
...
async function guardar(req, res) {
  const { checklist, tipoDiagnostico, costoDiagnostico, subtotalManoObra, subtotalRepuestos } = req.body;

  if (tipoDiagnostico && !["superficial", "profundo"].includes(tipoDiagnostico)) {
    return res.status(400).json({ error: "tipoDiagnostico debe ser 'superficial' o 'profundo'." });
  }
...
async function enviarAAprobacion(req, res) {
  try {
    const diagnostico = await diagnosticoDao.enviarAAprobacion(req.params.idOrden, req.usuario.idUsuario);
